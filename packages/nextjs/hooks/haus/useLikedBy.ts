...
import { readAllHausId } from "~~/utils/crud/crud-haus-swipes";

/**
 * useFeed HOOK
 * Use this to get feed of videos
 **/
export const useLikedBy = () => {
  ...
  const fetchFeed = async () => {
    try {
      ...
      const { data, error } = await readAllHausId(haus.haus.id);

      if (error) {
        console.error("Error fetching swiped data:", error);
        return;
      }
